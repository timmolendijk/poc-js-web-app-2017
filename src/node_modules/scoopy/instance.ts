...
export default class Instance {
  ...
  protected constructor(private readonly wrapped: Object) {
    ...
    // TODO(tim): Make this React dependency a plugin somehow.
    if (this.wrapped instanceof React.Component) {
      const store = Store.get(this.wrapped.context.store);
      // TODO(tim): Perhaps we should let the wrapper React component take the
      // identifier, and not burdening the wrapped component with it.
      const id = this.wrapped.props.id;
      if (store.hasInstance(getKeyOfType(this.Type), id))
        throw new Error("Every instance of a React component should have a unique identifier");
      // TODO(tim): Be more explicit about which property is used as an
      // identifier. Current auto-magic behavior is bound to be confusing.
