...
export function resetNextId(store: ReduxStore<IState>) {
  store.dispatch({
    type: `${NS}/RESET_NEXT_ID`
  });
}

export function reducer(state = {}, action) {
  if (action.type == `${NS}/RESET_NEXT_ID`)
    state = {
      ...state,
      [`${NS}/nextId`]: 1
    };
  if (action.type == `${NS}/INCREMENT_NEXT_ID`)
    state = {
      ...state,
      [`${NS}/nextId`]: state[`${NS}/nextId`] + 1
    };
...
export default class Store {
  ...
  getNextId(): number {
    const nextId = this.currentState[`${NS}/nextId`] as any;
    this.wrapped.dispatch({
      type: `${NS}/INCREMENT_NEXT_ID`
    });
    return nextId;
  }
