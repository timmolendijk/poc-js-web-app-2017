...
export default class Store {
  ...
  private hasObject(ref: IRef): boolean {
  ...
  private getObject(ref: IRef): Object {
    const refHash = getRefHash(ref);
    if (!this.objects.has(refHash) && this.hasObject(ref)) {
      const Type = getTypeOfKey(ref.type);
      this.objects.set(refHash, new Type());
    }
    return this.objects.get(refHash);
  }
