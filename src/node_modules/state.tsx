...
export function identifier() {
  return (target, name, descriptor) => {
    ...
    oldField()(target, name, descriptor);
  };
}
...
export function observable(decorator) {
  ...
  return function (config: IFieldConfig = {}) {
    ...
    const observable = mobx.observable(mobx.asReference(undefined));
    return decorator(Object.assign({
      onTracked(name) {
        const container = getContainer(this);
        observable.set(container.getField(this, name));
...
// TODO(tim): Ditch
export function oldField(dataToValue?, valueToData?, onTracked?) {
  return (target, name, descriptor) => {
    const Type = target.constructor;
    const typeRef = createTypeName(Type);
    if (!typeRef)
