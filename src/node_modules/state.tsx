...
export function field(dataToValue?, valueToData?, onTracked?) {
  return (target, name, descriptor) => {
    const Type = target.constructor;
    ...
    fields.push({ name, dataToValue, valueToData, onTracked });
  ...
  readonly enhancer = createStore => (reducer, preloadedState) => {
    ...
    this.store = createStore((state, action) => {
      ...
      if (action.type.startsWith('@@state/'))
        state['@@state'] = objectsReducer(state['@@state'], action);
      return state;
    }, preloadedState);
...
/*
  readonly observables = new Map<IReference, any>();

  observable(object, field) {
    const ref = this.createReference(object);
    if (!this.observables.has(ref) || !(field in this.observables.get(ref))) {
      const fields = this.observables.get(ref) || {};
      fields[field] = {
        subscribe: next => {
          // TODO(tim): Do we really need caching per *subscriber*? Expensive!
          next = memoize(next);
          this.store.subscribe(() =>
            next(this.getField(object, field))
          );
        }
      };
      this.observables.set(ref, fields);
    }
    return this.observables.get(ref)[field];
  }
*/
  ...
  createReference(object): IReference {
    ...
    if (!this.exists(ref)) {
      ...
      for (const { name, valueToData } of fields)
        data[name] = !valueToData ? object[name] :
          valueToData(object[name], this.createReference.bind(this));
      this.store.dispatch({
        type: '@@state/CREATE',
        object: ref,
        data
    ...
    const fields = typeToFields.get(Type);
    for (const { name, onTracked } of fields)
      if (onTracked)
        onTracked.call(object);
